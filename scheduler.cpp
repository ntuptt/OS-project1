...
int Scheduler::scheduling(struct process *proc, int nproc) {
	...
	while(1) {
		...
		int next = next_process(proc, nproc);
		if (next != -1) {
			/* Context switch */
			if (running != next) {
...
int PSJF_Scheduler::next_process(struct process *proc, int nproc) {
	int ret = -1;
...
int SJF_Scheduler::next_process(struct process *proc, int nproc) {
	if (running != -1) {
		return running;
	}
	int ret = -1;
...
int RR_Scheduler::next_process(struct process *proc, int nproc) {
	int ret = -1;
...
int FIFO_Scheduler::next_process(struct process *proc, int nproc) {
	if (running != -1) {
		return running;
	}
	int ret = -1;
